...
class DetectionOverlay:
    ...
    def __init__(self, args):
        ...
        self.font = ImageFont.truetype("OpenSans-Regular.ttf", 12)
    ...
    def get_bbox_tuples(self, feature):
        ...
        if self.args.bbox_name_key in feature:
            ...
            print("Bounding box key '%s' not present." %
                  (self.args.bbox_name_key))
        return bboxes
    ...
    def draw_bboxes(self, image_bytes, bboxes):
        ...
        for bbox in bboxes:
            label, xmin, xmax, ymin, ymax = self.bboxes_to_pixels(
                bbox, width, height)
            draw.rectangle([xmin, ymin, xmax, ymax],
                           outline=self.bbox_color(label), width=3)

            if self.args.showLabels:
                w, h = self.font.getsize(label)
                draw.rectangle(
                    (xmin, ymin, xmin + w + 4, ymin + h), fill="white")
                draw.text((xmin+4, ymin), label,
                          fill=self.bbox_color(label), font=self.font)
