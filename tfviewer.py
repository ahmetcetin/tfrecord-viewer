...
parser.add_argument('tfrecords', type=str, nargs='+', default="./*.tfrecord",
...
def preload_images(max_images):
    ...
    mylist = [f for f in glob.glob(args.tfrecords[0])]

    for tfrecord_path in mylist:
        ...
        for i, record in enumerate(tf.python_io.tf_record_iterator(tfrecord_path)):
            ...
            if len(images) < max_images:
                filename = feat[args.filename_key].bytes_list.value[0].decode(
                    "utf-8")
                img = feat[args.image_key].bytes_list.value[0]

                img_with_overlay = overlay.apply_overlay(img, feat)

                imgdict[os.path.basename(tfrecord_path)] = img_with_overlay
...
def get_image_by_filename(filename):
    ...
    """
    # key = int(key)
    img = imgdict[filename]
    img_buffer = io.BytesIO(img)
    return send_file(img_buffer,
                     attachment_filename=filename+'.jpeg',
                     mimetype='image/jpg')
